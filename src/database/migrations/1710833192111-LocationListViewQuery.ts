...
export class LocationListViewQuery1710833192111 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
            ...
            json_build_object('first_name', volunteer.first_name, 'last_name', volunteer.last_name, 'tenant_id', volunteer.tenant_id) AS site_contact_info,
           ...
           FROM crm_locations
             LEFT JOIN crm_volunteer volunteer ON volunteer.id = crm_locations.site_contact_id AND volunteer.is_archived = false
             LEFT JOIN address ON crm_locations.id = address.addressable_id AND address.addressable_type::text = 'crm_locations'::text
             LEFT JOIN qualifications qualification ON qualification.location_id = crm_locations.id AND qualification.qualification_status = true AND crm_locations.is_archived = false
