import { ContactsNoteSubCategoryModule } from './../note_subcategory.module';
import { ContactsNoteCategoryModule } from '../../note-category/note_category.module';
...
describe.skip('Note Sub category', () => {
  ...
  beforeAll(async () => {
    ...
    note = await noteCategoryRepository.save({
      name: 'Test',
      description: 'Test',
      type: typeEnum?.CRM_CONTACTS_NOTES,
      is_active: true,
  ...
  describe('- Validate User Before Creating Note Sub Category', () => {
    it('It Should Authenticate user Before Creating Note Sub category', async () => {
      const { status } = await supertest
        ...
        .post('/contacts/note-subcategory')
        .send({})
  ...
  describe('- Retrieve Sub Note By Id', () => {
    it('Should return a specific Note Category', async () => {
      const { body } = await supertest
        .agent(app.getHttpServer())
        .get(`/contacts/note-subcategory/${note_sub_id}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${access_token}`)
        .expect('Content-Type', /json/)
        .expect(200);
  ...
  describe('- update note sub category', () => {
    it('Should Update a note Category', async () => {
      ...
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .put(`/contacts/note-subcategory/${note_sub_id}`)
        .send(noteData)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/);
  ...
  describe('- Archive note Sub Category', () => {
    it('Should archive an note sub Category', async () => {
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .patch(`/contacts/note-subcategory/${note_sub_id}`)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/)
        .expect(200);
