import { LocationsNoteCategoryModule } from '../../note-category/note_category.module';
import { LocationNoteSubCategoryModule } from '../note_subcategory.module';
...
describe.skip('Note Sub category', () => {
  ...
  beforeAll(async () => {
    ...
    note = await noteCategoryRepository.save({
      name: 'Test',
      description: 'Test',
      type: typeEnum?.CRM_LOCATION_NOTES,
      is_active: true,
  ...
  describe('- Validate User Before Creating Note Sub Category', () => {
    it('It Should Authenticate user Before Creating Note Sub category', async () => {
      const { status } = await supertest
        .agent(app.getHttpServer())
        .post('/locations/note-subcategory')
        .send({})
  ...
  describe('- Retrieve Sub Notes', () => {
    it('Should return an array of Note Sub Category', async () => {
      const { body } = await supertest
        .agent(app.getHttpServer())
        .get('/locations/note-subcategory')
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${access_token}`)
        .expect('Content-Type', /json/)
        .expect(200);
  ...
  describe('- Retrieve Sub Note By Id', () => {
    it('Should return a specific Note Category', async () => {
      const { body } = await supertest
        .agent(app.getHttpServer())
        .get(`/locations/note-subcategory/${note_sub_id}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${access_token}`)
        .expect('Content-Type', /json/)
        .expect(200);
  ...
  describe('- update note sub category', () => {
    it('Should Update a note Category', async () => {
      ...
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .put(`/locations/note-subcategory/${note_sub_id}`)
        .send(noteData)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/);
  ...
  describe('- Archive note Sub Category', () => {
    it('Should archive an note sub Category', async () => {
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .patch(`/locations/note-subcategory/${note_sub_id}`)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/)
        .expect(200);
