...
import { NotesAttachmentCategoryModule } from '../attachment-category.module';
...
describe.skip('Attachment category', () => {
  ...
  describe('- Create Account', () => {
    it('It Should Authenticate user Before Creating Note category', async () => {
      const { status, body } = await supertest
        ...
        .post('/notes-attachments/attachment-categories/')
        .send({})
  ...
  it('Should create a attachment category', async () => {
    const noteData: any = {
      name: 'Test',
      description: 'Testing',
      is_active: true,
  ...
  describe('- Retrieve Notes', () => {
    it('Should return an array of Note Category', async () => {
      const { body } = await supertest
        .agent(app.getHttpServer())
        .get('/notes-attachments/attachment-categories')
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${access_token}`)
        .expect('Content-Type', /json/)
        .expect(200);
  ...
  describe('- Retrieve Note', () => {
    it('Should return a specific Note Category', async () => {
      const { body } = await supertest
        .agent(app.getHttpServer())
        .get(`/notes-attachments/attachment-categories/${note_id}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${access_token}`)
        .expect('Content-Type', /json/)
        .expect(200);
  ...
  describe('- update note category', () => {
    it('Should Update a note Category', async () => {
      ...
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .put(`/notes-attachments/attachment-categories/${note_id}`)
        .send(noteData)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/);
  ...
  describe('- Archive note Category', () => {
    it('Should archive an note Category', async () => {
      const { body, status } = await supertest
        .agent(app.getHttpServer())
        .patch(`/notes-attachments/attachment-categories/${note_id}`)
        .set('Authorization', `Bearer ${access_token}`)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/);
